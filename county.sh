...
echo "Confirmed,County" >county_count.csv ; node nyt2   2>/dev/null | jq '.u[]."County where treated"' -r | uniq -c | tr -s ' ' | sed 's/^ //g' | sed 's/ /,/' >> county_count.csv
...
csvjoin -c '1,1' nyt_us_county_upper.csv california_icu_data.csv -I  | csvcut -c 'COUNTY,POPULATION_2010,13,14,ICU BED COUNT' > california.csv
csvjoin -c 'COUNTY,COUNTY' california.csv california_hospital_count_county.csv -I | sponge california.csv


csvstack -g "`date +%s`" -n unixtime california.csv |csvstack -g "`date`" -n date2 | csvcut -c "3,4,5,6,7,1,2" | tee -a ca_county/ca_county_$(date +%s).csv | xsv table
